...
const Navbar = () => {
  ...
  const {isLoggedIn,setIsLoggedIn,setLoginFormData,selected,setSelected,openLogin,setOpenLogin,openSignUp,setOpenSignUp, scrollToSection, homeRef, aboutRef, contactRef }=useContext(AppContext);
  ...
  useEffect(() => {
    ...
    const options = {
      root: null,
      threshold: 0.5, // Lower threshold to ensure it triggers earlier
    };
  
    const observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          setSelected(entry.target.getAttribute('data-section'));
        }
      });
    }, options);
        ...
        <div className='flex'>
          ...
          {/* services button for mobile view */}
          {
            ...
            <div onClick={()=> {
              ...
              <div className={` ${dropDown ? 'text-blue-500 underline' : 'text-white'} ${selected==='service-icon' ? 'text-blue-400 underline underline-offset-8 decoration-blue-400' : 'text-white'} text-lg hover:underline underline-offset-8`}>Services</div>
              <div>
                {
          ...
          isLoggedIn && dropDown && (
            <div className={`fixed right-[60px] top-[55px] lg:hidden w-3/12 h-auto z-30 bg-white p-2 rounded-sm shadow-md transition-transform duration-300 ${dropDown ? 'transform translate-y-0' : 'transform -translate-y-full'}`}>
              <div className='text-center flex flex-col space-y-2 text-sm text-black'>
                <div>
                  <NavLink to='/diet' onClick={()=>{
                    setDropDown(false);
                    setSelected('service-icon');
                    }} className="hover:text-blue-700 transition-all duration-200 " >Diet Plan</NavLink>
                </div>
                ...
                <div>
                  ...
                  className="hover:text-blue-700 transition-all duration-200 ">Ask Ai</NavLink>
                </div>
              </div>
            </div>
          )
            ...
            <div id="menucontainer" className="w-screen h-screen"
              ...
              <div className={`fixed w-4/12 right-[10px] top-[10px] rounded-sm bg-white p-2 text-black text-lg flex flex-col justify-center items-center space-y-1 md:hidden transition-transform duration-300 ${openMenu ? 'transform translate-x-0' : 'transform translate-x-full'}`}>
                ...
                <div>
                  <div onClick={()=>{
                    setOpenMenu(false);
                    }} className="hover:text-blue-700 transition-all duration-200 hover:cursor-pointer ">
                    About
                  </div>
                </div>
                <div className="bg-black h-[0.6px] w-full opacity-10"></div>
        ...
        <section className='hidden text-lg md:flex justify-between mx-auto items-center roboto-bold'>
          <ul className='flex gap-x-6'>
            ...
            <li><NavLink onClick={()=>{
              setSelected('home');
              scrollToSection(homeRef);
            }} className={`${ selected==='home' ? "text-blue-400 underline underline-offset-8 decoration-blue-400" : "text-white"}  hover:underline decoration-1 underline-offset-8 transition-all duration-300`} to="/">HOME</NavLink></li>
            ...
            
            {
              ...
              }} className={`${ selected==='about' ? "text-blue-400 underline underline-offset-8 decoration-blue-400" : "text-white"} hover:cursor-pointer  hover:underline decoration-1 underline-offset-8 transition-all duration-300`}>ABOUT</li>
            }
            {
              !isLoggedIn &&
              <li onClick={()=>{
              ...
              }} className={`${ selected==='contact' ? "text-blue-400 underline underline-offset-8 decoration-blue-400" : "text-white"} hover:cursor-pointer hover:underline decoration-1 underline-offset-8 transition-all duration-300`} >CONTACT</li>
            }
            {
              isLoggedIn &&
        ...
        <div className='hidden lg:flex justify-between items-center roboto-bold gap-x-4 text-lg mr-3'>
          ...
          }
          {
            ...
            <div><NavLink onClick={()=>{
              setSelected('ai');
              setScrolled(true);
            }} className={`${selected==='ai' ? "text-blue-400 underline underline-offset-8 decoration-blue-400" : "text-white"}  uppercase hover:underline decoration-1 underline-offset-8 transition-all duration-300 flex wrap`}
